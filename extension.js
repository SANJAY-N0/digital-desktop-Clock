...
export default class DesktopDigitalClockExtension extends Extension {
    enable() {
        this._configFile = GLib.build_filenamev([GLib.get_user_config_dir(), 'desktop-digital-clock.json']);
        ...
        this._container.add_child(this._topLabel);
        ...
        this._container.add_child(this._bottomLabel);
    ...
    disable() {
        ...
        if (this._topLabel) { this._topLabel.destroy(); this._topLabel = null; }
        if (this._timeLabel) { this._timeLabel.destroy(); this._timeLabel = null; }
        if (this._bottomLabel) { this._bottomLabel.destroy(); this._bottomLabel = null; }
    ...
    _readConfigAsync() {
        ...
        file.load_contents_async(null, (obj, res) => {
            try {
                ...
                this._dragMode = data.drag_mode ?? true;
                this._timezone = data.timezone || "Local";
                this._fontSize = data.font_size || 32;
                this._topText = data.top_text ?? "CT";
                this._bottomText = data.bottom_text ?? "+9";
    ...
    _writePositionToConfig(posX, posY) {
        let data = {
            ...
            top_text: this._topText,
            bottom_text: this._bottomText,
            x: posX,
            y: posY
        };
        let file = Gio.File.new_for_path(this._configFile);
        ...
        file.replace_contents_bytes_async(bytes, null, false, Gio.FileCreateFlags.NONE, null, (obj, res) => {
            try {
                obj.replace_contents_bytes_finish(res);
                this._savedX = posX;
                this._savedY = posY;
            } catch (e) {}
        });
    ...
    _updateTimeText() {
        ...
        let metaHexColor = this._bgIsWhite ? 'rgba(15,15,15,0.5)' : 'rgba(255,255,255,0.5)';

        if (this._topLabel) {
            this._topLabel.set_text(this._topText);
            this._topLabel.set_style(`color: ${metaHexColor}; font-size: 11pt; font-weight: bold; font-family: sans-serif-condensed, sans-serif; text-transform: uppercase;`);
        }

        if (this._timeLabel) {
            this._timeLabel.set_text(`${hours}:${minutes}`);
            this._timeLabel.set_style(`color: ${textHexColor}; font-size: ${this._fontSize}pt; font-weight: 800; font-family: "Nimbus Sans Sync", "Arial Condensed", sans-serif-condensed, sans-serif; font-stretch: condensed; text-shadow: 0px 1px 2px rgba(0,0,0,0.15);`);
        }

        if (this._bottomLabel) {
            this._bottomLabel.set_text(this._bottomText);
            this._bottomLabel.set_style(`color: ${metaHexColor}; font-size: 11pt; font-weight: bold; font-family: sans-serif-condensed, sans-serif;`);
        }
    }
